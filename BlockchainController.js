...
class BlockchainController {
    ...
    this.app.get("/block/all", async (req, res) => {
      try {
        ...
        if (blocks) {
          ...
          return res.status(404).send("Chain is empty!");
        }
      } catch (error) {
  ...
  getBlockByHash() {
    this.app.get("/block/hash/:hash", async (req, res) => {
      if (req.params.hash) {
        ...
        debugger;
        let formattedBlocks = block.map((a) => ({
          owner: a.owner,
          star: a.getBData(),
        }));
    ...
    this.app.get("/blocks/:address", async (req, res) => {
      if (req.params.address) {
        ...
        try {
          ...
          if (stars.length > 0) {
            let formattedStars = stars.map((a) => ({
              owner: a.owner,
              star: a.getBData(),
            }));

            return res.status(200).json(formattedStars);
          } else {
            return res.status(404).send("Block Not Found!");
          }
        } catch (error) {          
  ...
  validateChain() {
    this.app.get("/validatechain", async (req, res) => {
      try {
        debugger;
        var result = await this.blockchain.validateChain();
